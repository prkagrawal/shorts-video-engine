 ...
 */
async function encodeFrame(canvas, encoder, timestamp, keyFrame = false) {
 ...
 */
async function addSegment(canvas, ctx, renderFn, durationSecs, fps, encoder, startTimestamp, frameCounter) {
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    await encodeFrame(canvas, encoder, startTimestamp + f * frameDuration, keyFrame);
    frameCounter.count++;
  }
...
export async function generateVideo(quiz, config, onProgress) {
  ...
  ts = await addSegment(canvas, ctx, (c) => renderIntro(c, quiz.title, quiz.questions.length),
    introDuration, fps, encoder, ts, frameCounter);
  progress();
  ...
  for (let p = 0; p < answerKeyPages; p++) {
    ts = await addSegment(canvas, ctx,
      ...
      answerKeyDuration, fps, encoder, ts, frameCounter);
    progress();
  }
  ...
  ts = await addSegment(canvas, ctx, (c) => renderOutro(c, null, quiz.questions.length),
    outroDuration, fps, encoder, ts, frameCounter);
  progress();
