...
function renderIntro(ctx, title, totalQuestions) {
  ...
  ctx.fillText("Let's see how many you get right!", cx, midY + 130);
...
function renderThink(ctx, question, qNum, total) {
  ...
  ctx.font = 'bold 72px sans-serif';
  ctx.fillStyle = TEXT;
  ctx.fillText('🤔', cx, midY - 60);
  ctx.fillText('Think…', cx, midY + 80);

  ctx.font = '40px sans-serif';
  ctx.fillStyle = MUTED;
  ctx.fillText("What's your answer?", cx, midY + 180);
...
export async function generateVideo(quiz, config, onProgress) {
  ...
  const totalSegments = 1 + quiz.questions.length * 3 + 1; // intro + (q+think+answer)*n + outro
  let doneSegments = 0;
  const progress = () => {
    doneSegments++;
