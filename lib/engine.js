...
const MIN_QUESTION_DURATION = 3.0;
const MAX_QUESTION_DURATION = 8.0;
const MIN_ANSWER_DURATION   = 2.5;
const MAX_ANSWER_DURATION   = 5.0;
const DYNAMIC_THINK_DURATION = 2.0;
...
function computeQuestionDuration(question) {
  const words = wordCount(question.question) +
    question.options.reduce((s, o) => s + wordCount(o), 0);
...
/** Draw a bold, low-opacity centred watermark. */
...
function drawPromoBanner(ctx) {
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  // Scale font so text fits within canvas width (with 20px padding each side)
  const maxW = W - 40;
  let size = 32;
  ctx.font = `bold ${size}px sans-serif`;
