...
// Keyframe interval: one keyframe every 30 frames (= 1 s at 30 fps)
 ...
 *   keyframes land every KEYFRAME_INTERVAL frames across the whole video.
 */
async function addSegment(canvas, ctx, renderFn, durationSecs, fps, encoder, startTimestamp, frameCounter) {
  ...
  for (let f = 0; f < totalFrames; f++) {
    const keyFrame = frameCounter.count % KEYFRAME_INTERVAL === 0;
