...
function renderAnswer(ctx, question, qNum, total) {
  ...
  const optionFont = '46px sans-serif';
  const labelR = 30;
  const labelGap = 20;
  const checkmarkSpace = 60; // horizontal space reserved for the ✓ badge
  const checkmarkPad = 10;   // gap between ✓ and the card right edge
  const textStartX = PAD + 18 + labelR * 2 + labelGap;
  const optionTextMaxW = W - 2 * PAD - (18 + labelR * 2 + labelGap) - checkmarkSpace;
  const optionPadV = 28;
  const optionGap = 20;
  let optY = qBottom + 50;
  ...
  question.options.forEach((text, i) => {
    ...
    // Measure how tall this option's text will be when wrapped
    const textH = measureWrappedText(ctx, text, optionFont, optionTextMaxW);
    const optionH = Math.max(100, textH + 2 * optionPadV);

    const ox0 = PAD;
    const oy0 = optY;
    const optW = W - 2 * PAD;
    const cxc = ox0 + 18 + labelR;
    const cyc = oy0 + optionH / 2;

    // Dim incorrect options
    if (!isCorrect) ctx.globalAlpha = 0.4;

    // Card background
    roundRect(ctx, ox0, oy0, optW, optionH, 24,
      isCorrect ? 'rgba(0,184,148,0.85)' : 'rgba(255,255,255,0.08)');

    // Correct answer border highlight
    if (isCorrect) {
      ctx.strokeStyle = CORRECT;
      ctx.lineWidth = 4;
      const r = 24;
      ctx.beginPath();
      ctx.moveTo(ox0 + r, oy0);
      ctx.lineTo(ox0 + optW - r, oy0);
      ctx.quadraticCurveTo(ox0 + optW, oy0, ox0 + optW, oy0 + r);
      ctx.lineTo(ox0 + optW, oy0 + optionH - r);
      ctx.quadraticCurveTo(ox0 + optW, oy0 + optionH, ox0 + optW - r, oy0 + optionH);
      ctx.lineTo(ox0 + r, oy0 + optionH);
      ctx.quadraticCurveTo(ox0, oy0 + optionH, ox0, oy0 + optionH - r);
      ctx.lineTo(ox0, oy0 + r);
      ctx.quadraticCurveTo(ox0, oy0, ox0 + r, oy0);
      ctx.closePath();
      ctx.stroke();
    }

    // Label circle – white for correct (stands out on green), OPTION_COLORS for incorrect
    ctx.beginPath();
    ctx.arc(cxc, cyc, labelR, 0, Math.PI * 2);
    ctx.fillStyle = isCorrect ? '#ffffff' : OPTION_COLORS[i % OPTION_COLORS.length];
    ctx.fill();
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'top';
    drawWrappedText(ctx, text, textStartX, oy0 + optionPadV, optionFont,
      isCorrect ? '#ffffff' : TEXT, optionTextMaxW);

    // Checkmark badge on the correct option
    if (isCorrect) {
      ctx.font = 'bold 44px sans-serif';
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillStyle = '#ffffff';
      ctx.fillText('✓', W - PAD - checkmarkPad, cyc);
    }

    // Restore full opacity before next iteration
    ctx.globalAlpha = 1;
    optY += optionH + optionGap;
  });
