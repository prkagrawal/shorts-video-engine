...
/**
 * Maximum number of frames allowed in the encoder queue before we pause
 * submission.  Without this cap the caller can flood the encoder queue faster
 * than it drains (especially with software encoding on desktop), causing the
 * browser to appear stuck while flush() works through the backlog.
 */
const QUEUE_LIMIT = 5;
 ...
 *
 * Applies back-pressure: if the encoder's internal queue already holds
 * QUEUE_LIMIT frames we wait for it to drain before submitting more.
 * This keeps memory bounded and lets progress reporting reflect *actual*
 * encoding progress rather than just queue-submission progress.
 *
 * @param {object} errorRef - { value: DOMException|null } set by the encoder
 *   error callback so we can surface the real failure reason.
 */
async function encodeFrame(canvas, encoder, timestamp, keyFrame, errorRef) {
  while (encoder.encodeQueueSize > QUEUE_LIMIT) {
    await new Promise(r => setTimeout(r, 10));
    if (errorRef.value) throw errorRef.value;
    if (encoder.state !== 'configured') {
      throw new Error('Video encoding failed — encoder is no longer active.');
    }
  }
 ...
 *
 * @param {object} frameCounter - { count: number } shared across segments so
 *   keyframes land every `fps` frames (once per second) across the whole video.
 */
async function addSegment(canvas, ctx, renderFn, durationSecs, fps, encoder, startTimestamp, frameCounter, errorRef) {
  const totalFrames = Math.round(durationSecs * fps);
  const frameDuration = 1_000_000 / fps; // µs per frame
  const keyframeInterval = fps; // one keyframe per second
  renderFn(ctx);
  for (let f = 0; f < totalFrames; f++) {
    const keyFrame = frameCounter.count % keyframeInterval === 0;
    await encodeFrame(canvas, encoder, startTimestamp + f * frameDuration, keyFrame, errorRef);
    frameCounter.count++;
  }
...
export async function generateVideo(quiz, config, onProgress) {
  ...
  // --- Codec support check with hardware-acceleration preference ---------
  const codecConfig = {
    codec: 'avc1.4D0028',
    width: W,
    height: H,
  ...
  const errorRef = { get value() { return encoderError; } };
  ...
  ts = await addSegment(canvas, ctx, (c) => renderIntro(c, quiz.title, quiz.questions.length),
    introDuration, fps, encoder, ts, frameCounter, errorRef);
  progress();
  ...
  for (let i = 0; i < quiz.questions.length; i++) {
    ...
    ts = await addSegment(canvas, ctx, (c) => renderQuestion(c, q, qNum, total),
      qDuration, fps, encoder, ts, frameCounter, errorRef);
    progress();

    ts = await addSegment(canvas, ctx, (c) => renderThink(c, q, qNum, total),
      tDuration, fps, encoder, ts, frameCounter, errorRef);
    progress();

    ts = await addSegment(canvas, ctx, (c) => renderAnswer(c, q, qNum, total),
      aDuration, fps, encoder, ts, frameCounter, errorRef);
    progress();
  }
  ...
  for (let p = 0; p < answerKeyPages; p++) {
    ts = await addSegment(canvas, ctx,
      ...
      answerKeyDuration, fps, encoder, ts, frameCounter, errorRef);
    progress();
  }
  ...
  ts = await addSegment(canvas, ctx, (c) => renderOutro(c, null, quiz.questions.length),
    outroDuration, fps, encoder, ts, frameCounter, errorRef);
  progress();
